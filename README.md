...
- 挿入数で並べた観測・caseと、全挿入に占める累積割合
- 句読点・記号を除いた診断用CER
- 固有名詞・言語の成功数と分母、分割数
- 確定遅延、境界の種別・理由、CPU、packet欠落
- RMS、peak、音割れ率、無音率、原文confidence、confidence閾値別の見逃し率・誤検知率

レポートversion 2では、NFKC正規化後に空白を除いてCERを計算する。`cer`は互換用のmicro CERであり、全観測の編集回数を全正解文字数で割った値である。`macro_cer`は観測ごとのCERの平均である。主CERは句読点と記号を誤りとして数える。`punctuation_and_symbol_insensitive_score`は、Unicodeの句読点と記号も除いた診断値であり、採用gateの主CERを置き換えない。編集内訳は同じ最小編集距離に複数の対応がある場合があるため、置換、削除、挿入の順で決める診断値として扱う。
