...
誤認識しやすい単語を音声認識の候補に加える場合は、`SONIOX_RECOGNITION_TERMS_JSON`を設定する。たとえば`{"ja-ko":["星見工房"]}`は、日本語・韓国語のセッションでSonioxの`context.terms`へ渡す。省略時は空で、指定していない言語ペアには追加しない。個人の用語はGit管理外の`.env.local`へ保存する。変更はBotを再起動した後のセッションから反映される。

この設定は認識候補への補助であり、特定の文字起こしを強制するものではない。翻訳用の`/register`とは別に扱い、話者の言語設定は維持する。認識用語彙・会話説明・翻訳用語を合算したcontextが10,000文字を超える場合は、起動前または用語登録時に拒否する。適用前に、誤認識した音声と正常な音声の両方で結果を比較する。
