...
`/language`はGuildと利用者の組み合わせごとにSQLiteへ保存する。`user`を省略すると自分を対象にする。他の利用者を確認または変更するには`ManageGuild`権限が必要である。保存した設定は`SPEAKER_LANGUAGE_HINTS`より優先される。「自動判定」を保存すると、そのGuildでは環境設定を使わず、選択した言語ペアの2言語をSonioxへ渡す。設定言語がセッションの言語ペアに含まれない場合も、自動判定へ戻す。
