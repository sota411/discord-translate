...
Sonioxの原文認識に、Pi上で動くDolphinの自動認識結果を加える実装です。**新たな改善を確認できたのは、韓国語の対照音声1本です。日本語の対照は維持していますが、「編み物」と「膝」の誤認識は解消できていません。**
...
Dockerイメージにモデルと実行ファイルを含めています。`compose.pi.yaml` では補助認識を有効にし、日本語・韓国語ペアへ適用します。`STT_LOCAL_REFINEMENT_ENABLED=false` を明示すると無効にできます。アプリ単体や通常のComposeの既定値は `false` です。ローカルで `pnpm dev` を実行する環境では、実行ファイルの別途配置が必要です。
