...
発話中は、認識途中の原文と翻訳文を仮字幕として同じメッセージへ最短500 ms間隔で反映し、表示内容が変わらない場合は再編集しない。編集間隔は、Discordが応答ヘッダーで返す残り回数とリセットまでの時間に合わせてスレッド単位で延ばす。discord.jsの`response`と`rateLimited`イベントを使い、再試行は引き続きSDKへ任せる。制限値は固定しない（[Discord Rate Limits](https://docs.discord.com/developers/topics/rate-limits)）。

Discordへの更新が遅い場合や編集を待機する場合は、発話ごとに実行中の1件と最新の待機中1件だけを保持する。古い待機中の仮字幕は破棄し、確定字幕または削除は仮字幕の待機タイマーを取り消して優先する。初回の字幕投稿は別の発話の遅い操作を待たない。発話が確定すると、そのメッセージを確定字幕へ置き換える。言語ペア内の原文が確定していれば、訳文がなくても原文を残し、「訳文を取得できませんでした。」と表示する。この字幕も`/translate export`の対象とする。原文が確定していない場合は仮字幕を削除する。
...
`endpoint`または`finalized`が1発話の境界になる。言語ペア内の原文が確定した発話は、訳文の有無によらず字幕へ渡す。訳文が空の場合はTTSを呼ばず、原文のみの確定字幕を投稿する。原文と訳文が揃った場合は、確定字幕の投稿とTTS生成を並行して始める。字幕投稿の完了はTTS生成開始の条件にしない。
