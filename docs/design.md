...
レポートversion 2では、NFKC正規化後に空白を除いて編集距離を求める。`cer`と`micro_cer`は同じ値であり、全観測の編集回数を全正解文字数で割る。`macro_cer`は観測ごとのCERを平均する。置換・削除・挿入の組み合わせが複数成立するときは、この順で選ぶ。編集総数は一意である。内訳は再現可能な診断値として使い、言語学的な誤り分類とはみなさない。
...
Aでは、`ja-clean-game`の3試行だけで42正解文字に147文字の挿入、`ja-clipped`で33正解文字に78文字の挿入、`ja-ko-code-switch`で27正解文字に51文字の挿入があった。この3 caseでAの挿入285文字中276文字を占める。CER 1超は計算上成立し、今回は挿入の多さを反映している。一方、短い10件だけに偏った結果なので、実会話の絶対精度としては扱わない。
...
保存済みobservationsには、受信イベント単位のtoken列と時刻が残っていない。したがって、今回の90観測に確定tokenの再送がなかったことは遡って証明できない。監査で確認できたのは、保存本文から再計算した編集距離と、その本文を作る現在のrunner経路である。再採点結果は[本文非含有のCER監査レポート](./evaluation/stt-recognition-catalog-scoring-audit-2026-08-26.json)へ残す。
