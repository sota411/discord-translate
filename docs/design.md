...
- 挿入数で降順に並べた観測・case、micro CERへの寄与、累積挿入割合
- Unicodeの句読点と記号を除いた診断用CER
- 固有名詞・期待言語の成功数と分母、分割数
- 確定遅延、境界の種別・確定理由・本文有無
- CPU、復号・欠落packet数、音質4指標、原文confidence
- baselineのconfidence閾値別の誤り再現率、誤検知率、追加処理率
- provider比較では、本文を取得できた観測の割合

レポートversion 2では、NFKC正規化後に空白を除いて編集距離を求める。`cer`と`micro_cer`は同じ値であり、全観測の編集回数を全正解文字数で割る。`macro_cer`は観測ごとのCERを平均する。主CERは句読点と記号も誤りとして数える。`punctuation_and_symbol_insensitive_score`は、NFKC正規化後に空白、Unicodeの句読点、記号を除いて同じ計算を行う診断値である。採用gateには主CERを使い、診断値との入れ替えは行わない。置換・削除・挿入の組み合わせが複数成立するときは、この順で選ぶ。編集総数は一意である。内訳は再現可能な診断値として使い、言語学的な誤り分類とはみなさない。

baselineでは音質指標とCERのPearson相関係数、tag別とそれ以外のmicro CERも出す。同一音声のtrialを独立標本として水増しせず、相関はcase単位へ集約する。認識本文が一つもないcaseでは、失敗を集計から消さないよう最終境界を対象にする。候補とAのtrial数が違う観測は比較しない。比較の初期採用基準は次のとおりである。
...
Soniox endpoint比率と手動fallback比率は、発話確定経路を調べる診断値として残す。利用者が感じる精度、分割、遅延が基準を満たせば、手動確定が多いことだけを理由に不採用とはしない。過去のversion 1レポートにある`semantic_endpoint` gateは当時の判定履歴として保持し、version 2で作る比較では`not_evaluated`とする。

2026年8月25日には、静音、短い相づち、小音量、clip、キーボード・ファン相当のノイズ、600 msの間、日韓切り替え、ゲーム用語を含む人工音声10件をSoniox `stt-rt-v5`へ3回ずつ送った。入力manifestのSHA-256は`6d350a14157addc4415a4f8374ea66b0bca2d32e2d170deb5cf3e1e284c4dee2`である。Aの試行別CERが1.37〜1.74に振れたため、単発値ではなく30観測/profileの集計値で判定した。

| 候補 | CER相対改善 | 固有名詞再現率の差 | コードスイッチCER差（負は改善） | 切替時の期待言語再現率差 | p95追加遅延 | 不自然な分割の増加 | Soniox endpoint比率 | 判定 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| B | -2.0% | -8.3ポイント | -111.1ポイント | -16.7ポイント | +2 ms | 0件 | 対象外 | 全体CER、固有名詞、期待言語で不採用 |
| C | 46.8% | -8.3ポイント | -114.8ポイント | 0ポイント | +488 ms | 25件 | 67.3% | 固有名詞、遅延、分割で不採用 |
...
| Soniox endpoint比率 | 13.3% | 50.0% | 47.1% | 診断値。採否対象外 |

Cは全体CERと遅延を改善したが、目的だった固有名詞再現率を大きく落とし、言語切り替えも悪化させた。endpoint比率はBで50.0%、Cで47.1%だった。以前のlevel 1実験で得た56.4%との差は、異なるbatch間の振れである。version 1レポートには過半数を採用gateにした当時の判定を残すが、version 2では診断値に変更した。Cは精度gateだけでも事前不採用になるため、結論は変わらない。
...
#### CER監査では挿入誤りが1超の主因だった

保存済みの固有名詞カタログ比較observationsを、採点ロジックだけ更新して再採点した。外部STTへの再送信は行っていない。独立した発話内容は10件であり、3 profileを各3試行した90観測である。3試行は応答の振れを測るための反復であり、90種類の発話を評価したことにはならない。

| profile | micro CER | macro CER | 置換 | 削除 | 挿入 | 編集総数/正解文字数 | 認識文字数 | 固有名詞 | 言語切り替え |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| A 現行 | 1.601 | 1.433 | 51 | 101 | 285 | 437/273 | 457 | 9/12 | 3/6 |
| B level 1 | 0.872 | 0.914 | 53 | 120 | 65 | 238/273 | 218 | 9/12 | 3/6 |
| C level 1＋共通カタログ | 1.125 | 1.057 | 53 | 132 | 122 | 307/273 | 263 | 5/12 | 1/6 |

Aでは、`ja-clean-game`の3試行だけで42正解文字に147文字の挿入、`ja-clipped`で33正解文字に78文字の挿入、`ja-ko-code-switch`で27正解文字に51文字の挿入があった。この3 caseでAの挿入285文字中276文字、96.8%を占める。全編集437文字の65.2%が挿入で、挿入は正解273文字の104.4%に相当する。置換と削除だけでも152/273=0.557が残る。CER 1超は計算上成立し、今回は少数caseへ集中した挿入と、それ以外の編集の両方を反映している。一方、短い10件だけに偏った結果なので、実会話の絶対精度としては扱わない。
...
追加した`recognition_catalog_factorial`実験は、共通カタログと`endpoint_latency_adjustment_level=1`を同じbatchで直交比較する。

| ID | 共通カタログ | level 1 | 発話確定 |
|---|---|---|---|
| A | なし | なし | 現行の100 ms手動確定 |
| B | 全case共通 | なし | Aと同じ |
| C | なし | あり | 400 ms fallback |
| D | 全case共通 | あり | 400 ms fallback |

これにより、カタログ単独、level 1単独、両者の相互作用を同一batch内で分けられる。caseごとの正解語は渡さず、manifest全体の`key_terms`を重複除去した同じ語彙をBとDへ同じ順序で送る。現在の人工音声manifestでは3語になる。Soniox endpoint比率は診断値に留め、採用gateを`not_evaluated`とする。この実験は評価runnerにだけ追加しており、まだ外部STTへ実行していない。Pi、通常のDiscordセッション、本番設定には接続していない。

実測後の最終表では、A〜Dの4行に対してmicro CER、挿入、固有名詞の成功数/分母、言語切り替えの成功数/分母、p95を同じbatchから並べる。現在はBを同一batchで測っていないため、過去の別実験を継ぎ合わせた数値表は作らない。

#### confidenceだけでは低コストな再認識条件をまだ作れない

句読点・記号を除くCERが0より大きい観測を内容誤りとして、baseline 30観測の平均・最低confidenceを0.7、0.8、0.9で区切った。confidenceを取得できたのは24観測で、内容誤り15件、内容一致9件だった。原文tokenがない6観測はconfidenceもなく、6件とも内容誤りである。この判定は文字列だけを対象にするため、文字列が一致しても言語labelだけが誤る観測は検出しない。

| 条件 | confidenceありの誤りを検出 | 内容一致の誤検知 | confidenceなしも対象にした追加処理 | 全内容誤りを検出 |
|---|---:|---:|---:|---:|
| 平均confidence < 0.8 | 6/15 | 0/9 | 12/30 | 12/21 |
| 平均confidence < 0.9 | 15/15 | 1/9 | 22/30 | 21/21 |
| 最低confidence < 0.8 | 15/15 | 6/9 | 27/30 | 21/21 |

平均0.8未満では9件を見逃す。平均0.9未満は、この人工音声では全誤りを拾うが、confidenceなしを安全側で再認識へ回すと全発話の73.3%へ追加処理が必要になる。最低0.8未満は90.0%を追加処理へ回し、内容一致も3分の2を誤検知する。少数の人工音声から選んだ閾値は過適合するため、いずれも本番条件には採用しない。実Discord音声では、confidenceに加えて音声時間に対する文字数、反復、言語反転を候補にし、再認識対象の再現率、誤検知率、全発話に占める追加処理率を同時に測る。
...
本番の`stt_audio_quality`と同じ集計器を評価runnerでも使い、同じ人工音声10件のbaselineを3試行した。30観測すべてでRMS、peak、音割れ率、無音率を取得した。原文confidenceは24観測で取得した。残る6観測は、小音量と600 msの間を含む2ケースで原文token自体が返らなかったため、confidenceとの相関から除外した。

相関はtrialごとではなくcaseごとへ集約した。正の値は指標が大きいほどCERも大きい関係、負の値は指標が大きいほどCERが小さい関係を示す。

| 指標 | CERとの相関 | case数 | 解釈上の制約 |
|---|---:|---:|---|
| RMS | 0.478 | 10 | 音量だけの因果を示さない |
| peak | 0.272 | 10 | 音量だけの因果を示さない |
| 音割れ率 | 0.651 | 10 | 音割れありは1ケースだけ |
| 無音率 | 0.038 | 10 | この入力では関係がほぼ見えない |
| packet欠落率 | 評価不能 | 10 | 全ケースで欠落0件 |
| 平均confidence | -0.145 | 8 | 原文tokenなしの2ケースを除外 |
| 最低confidence | -0.759 | 8 | 原文tokenなしの2ケースを除外 |

noiseタグ2ケースのmicro CERは0.50で、非noise 8ケースの2.063を下回った。一方、clipタグ1ケースのCERは4.091だった。したがって、この評価セットではノイズを主要因とは判断せず、RNNoiseとDeepFilterNetを実装または依存関係へ追加しない。音割れと最低confidenceには誤認識との関係が見えたが、少数の人工音声による相関であり、因果関係を示さない。実Discord音声でケース数を増やすまでは、limiterや音量補正も標準採用しない。case・trial別指標、tag別CER、入力SHA-256は[音質相関レポート](./evaluation/stt-audio-quality-correlation-2026-08-26.json)に残す。
...
今後、入力品質の統計でノイズが主要因になった場合は、同じ手順で無加工PCMと前処理を比較する。ノイズ音声のCERを10%以上相対改善し、クリーン音声を悪化させない場合だけ標準採用する。現時点では無加工PCMが唯一の経路である。
