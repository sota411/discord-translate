...
本文非含有レポートには、CER、固有名詞再現率、期待言語の再現率、分割数、確定遅延の平均・p50・p95、受理した境界の種別と確定理由、CPU、復号・欠落packet数を出す。比較の初期採用基準は次のとおりである。
...
| C | 46.5% | +25ポイント | +33.3ポイント | 0ポイント | +479 ms | 9件 | 60.0% | コードスイッチ、遅延、分割で不採用 |
| D | 29.2% | 0ポイント | +66.7ポイント | 0ポイント | +483 ms | 0件 | 33.3% | 固有名詞、コードスイッチ、遅延、endpoint比率で不採用 |
