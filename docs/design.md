...
Discordへの更新が遅い場合や編集を待機する場合は、発話ごとに実行中の1件と最新の待機中1件だけを保持する。古い待機中の仮字幕は破棄し、確定字幕または削除は仮字幕の待機タイマーを取り消して優先する。初回の字幕投稿は別の発話の遅い操作を待たない。発話が確定すると、そのメッセージを確定字幕へ置き換える。確定結果が空なら仮字幕を削除する。
