...
本文非含有レポートには、trial数、観測件数、caseごとのtrial番号、CER、固有名詞再現率、期待言語の再現率、分割数、確定遅延の平均・p50・p95、受理した境界の種別・確定理由・本文有無、CPU、復号・欠落packet数を出す。確定遅延とSoniox endpoint比率は、本文のある境界だけを採否対象にする。認識本文が一つもないcaseでは、失敗を集計から消さないよう最終境界を対象にする。候補とAのtrial数が違う観測は比較しない。比較の初期採用基準は次のとおりである。
