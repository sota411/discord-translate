...
本文非含有レポートには、CER、固有名詞再現率、期待言語の再現率、分割数、確定遅延の平均・p50・p95、受理した境界の種別・確定理由・本文有無、CPU、復号・欠落packet数を出す。確定遅延とSoniox endpoint比率は、本文のある境界だけを採否対象にする。認識本文が一つもないcaseでは、失敗を集計から消さないよう最終境界を対象にする。比較の初期採用基準は次のとおりである。
...
| 候補 | CER相対改善 | 固有名詞再現率の差 | コードスイッチCER差 | 切替時の期待言語再現率差 | p95追加遅延 | 不自然な分割の増加 | Soniox endpoint比率 | 判定 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| B | 11.1% | -25ポイント | 0ポイント | -50ポイント | -24 ms | 0件 | 対象外 | 固有名詞と言語切り替えで不採用 |
| C | 46.5% | +25ポイント | +33.3ポイント | 0ポイント | +479 ms | 9件 | 68.4% | コードスイッチ、遅延、分割で不採用 |
| D | 29.2% | 0ポイント | +66.7ポイント | 0ポイント | +483 ms | 0件 | 10.0% | 固有名詞、コードスイッチ、遅延、endpoint比率で不採用 |
